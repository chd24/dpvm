/* работа с чёрно-красным деревом, T11.684-T20.048; $DVS:time$ */
...
prefix struct ldus_rbtree *ldus_rbtree_find(struct ldus_rbtree *root, struct ldus_rbtree *node) postfix { \
    ...
    \
prefix struct ldus_rbtree *ldus_rbtree_upper_bound(struct ldus_rbtree *root, struct ldus_rbtree *node) postfix { \
	root = _rbtree_ptr(root); \
	if (!root) return 0; \
	if (lessthen(node, root)) { node = ldus_rbtree_find(root->left,  node); return node ? node : root; } \
	if (lessthen(root, node)) return ldus_rbtree_find(root->right, node); \
	return root; \
} \
    \
prefix struct ldus_rbtree *ldus_rbtree_lower_bound(struct ldus_rbtree *root, struct ldus_rbtree *node) postfix { \
	root = _rbtree_ptr(root); \
	if (!root) return 0; \
	if (lessthen(node, root)) return ldus_rbtree_find(root->left,  node); \
	if (lessthen(root, node)) { node = ldus_rbtree_find(root->right, node); return node ? node : root; } \
	return root; \
} \
    \
